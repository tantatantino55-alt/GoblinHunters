...
public class PauseMenuDrawer {
    ...
    private int drawControlsSection(Graphics2D g2d, int cx, int cy, PauseController ctrl) {
        ...
        for (int i = 0; i < PauseController.ACTION_COUNT; i++) {
            int rowY = cy + i * rowH;
    ...
    private void drawActionIcon(Graphics2D g2d, int row, int x, int y, int size) {
        g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        switch (row) {
            case PauseController.ACTION_MOVE_UP    -> drawArrowIcon(g2d, x, y, size, 0);
            case PauseController.ACTION_MOVE_DOWN  -> drawArrowIcon(g2d, x, y, size, 1);
            case PauseController.ACTION_MOVE_LEFT  -> drawArrowIcon(g2d, x, y, size, 2);
            case PauseController.ACTION_MOVE_RIGHT -> drawArrowIcon(g2d, x, y, size, 3);
            case PauseController.ACTION_BOMB -> {
                BufferedImage img = SpriteManager.getInstance().getSprite("HUD_FIRE_SPELL", 0);
                drawSpriteIcon(g2d, img, x, y, size);
            }
            case PauseController.ACTION_AURA -> {
                BufferedImage img = SpriteManager.getInstance().getSprite("HUD_AURA_SPELL", 0);
                drawSpriteIcon(g2d, img, x, y, size);
            }
            case PauseController.ACTION_STAFF -> {
